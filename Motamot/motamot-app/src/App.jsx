...
// Groq API for sentence generation (free tier available)
const generateSentence = async (words) => {
  ...
  // Categorize words by type for better context
  ...
  // Random structure selector to encourage variety and humor
  ...
  const randomHint = structureHints[Math.floor(Math.random() * structureHints.length)];
  
  const prompt = `Tu es un professeur de français natif avec un bon sens de l'humour qui crée des phrases d'exemple amusantes pour des étudiants débutants.
...
function App() {
  ...
  const handleClick = useCallback(async () => {
    ...
    if (sentence || error) {
      // Reset after showing sentence or error
      setSentence(null);
      setError(null);
      setClickCount(0);
      setDisplayedWords([]);
    ...
    if (newClickCount >= CLICKS_BEFORE_SENTENCE) {
      ...
      try {
        ...
        setError(err.message || ERROR_API_FAILED);
        setSentence(null);
      } finally {
        setIsLoading(false);
      }
      ...
      <div className="word-container">
        {isLoading ? (
          <div className="loading">
            <span className="loading-text">Génération de la phrase...</span>
            <div className="loading-spinner"></div>
          </div>
        ) : (
          <span className={`word ${isErrorOrSentence ? 'sentence' : ''} ${error ? 'error' : ''}`}>
            {displayText}
          </span>
        )}
      </div>
        ...
        {!isErrorOrSentence && !isLoading && (
          <div className="progress-dots">
            {Array.from({ length: CLICKS_BEFORE_SENTENCE }).map((_, i) => (
              <span 
                key={i} 
                className={`dot ${i < clickCount ? 'filled' : ''}`}
              />
            ))}
          </div>
        )}
